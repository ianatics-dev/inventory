...
const PersonTable: React.FC<PersonTableProps> = ({ rows }) => {
  ...
  const handleSubmitTurnIn = async () => {
    ...
    try {
      ...
      const res = await app.post(`/api/guns/${gunId}/turn_in/`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      const updatedGun = res.data;

      toast.success("Turn in successful!");

      setUpdatedRows((prev) =>
        prev.map((r) => {
          const currentGunId = getGunId(r);
          if (currentGunId === gunId) {
            return {
              ...r,
              guns: r.guns ? updatedGun : r.guns,
              gun: r.gun ? updatedGun : r.gun,
            };
          }
          return r;
        })
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setUpdatedRows((prev) => prev.filter((r) => getGunId(r) !== gunId));
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      flex: 1,
      minWidth: 120,
      valueGetter: (_v, row: any) => row?.issued_to?.unit ?? "-",
    },
    {
      field: "sub_unit",
      headerName: "Sub Unit",
      flex: 1,
      minWidth: 150,
      valueGetter: (_v, row: any) => row?.issued_to?.sub_unit ?? "-",
    },
    {
      field: "station",
      headerName: "Station",
      flex: 1,
      minWidth: 150,
      valueGetter: (_v, row: any) => row?.issued_to?.station ?? "-",
    },
    {
      field: "issued_unit",
      headerName: "Issuing Unit",
      flex: 1.4,
      minWidth: 140,
      valueGetter: (_v, row: any) => row?.issued_to?.issued_unit ?? "-",
    },
    {
      field: "faid",
      headerName: "Faid",
      flex: 1,
      minWidth: 160,
      valueGetter: (_v, row: any) => getGun(row)?.faid ?? "",
    },
    {
      field: "serial_no",
      ...
      flex: 1,
      minWidth: 150,
      valueGetter: (_v, row: any) => getGun(row)?.serial_no ?? "",
    },
    {
      field: "baril",
      headerName: "Make / Model / Kind / Caliber",
      flex: 2,
      minWidth: 280,
      valueGetter: (_v, row: any) => {
        const g = getGun(row);
        if (!g) return "";
        return `${g.make ?? ""} / ${g.model ?? ""} / ${g.kind ?? ""} / ${g.caliber ?? ""}`.trim();
      },
    },
    {
      field: "validated",
      ...
      flex: 1,
      minWidth: 120,
      valueGetter: (_v, row: any) => getGun(row)?.validated ?? "",
    },
  ];
